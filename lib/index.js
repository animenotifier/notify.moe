...
arn.getAnimeList = function(user, callback) {
	...
	arn.get('AnimeLists', user.id).then(animeList => {
		...
		if(arn.cacheAnimeLists && cacheKey === animeList.cacheKey && now.getTime() - generated.getTime() < arn.animeListCacheTime) {
			callback(undefined, animeList)
		} else {
			refresh(animeList)
		}
...
arn.getAnimeIdBySimilarTitle = Promise.promisify(function(anime, callback) {
	if(!anime)
