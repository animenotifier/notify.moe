	...
	let refresh = (oldAnimeList) => {
		return arn.refreshAnimeList(user, listProvider, animeProvider, airingDateProvider, listProviderSettings, cacheKey).then(animeList => {
			...
			animeList.watching.forEach(anime => {
				let oldAnimeMatched = oldAnimeList.watching.filter(e => e.providerId === anime.providerId)

				if(!oldAnimeMatched)
					return

				let oldAnime = oldAnimeMatched[0]
				console.log(oldAnime.episodes.available, 'vs', anime.episodes.available)

				if(!user.devices)
					return

				// Send push notification
				if(anime.episodes.available && (!oldAnime.episodes.available || anime.episodes.available > oldAnime.episodes.available)) {
