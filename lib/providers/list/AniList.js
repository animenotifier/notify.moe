	...
	getAiringDate(anime) {
		...
		return request({
			...
			try {
				...
				throw new Error(`Error getting airing date for ${anime.title} (${searchTitle})`)
			}
		...
		.catch(error => {
			...
			if(error.statusCode !== 404 && !error.message.startsWith('Error getting airing date'))
				console.error(error.stack)
		})
	}
