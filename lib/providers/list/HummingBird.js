...
class HummingBird {
	...
	getAnimeList(userName, callback) {
		...
		return request({
			uri: `https://hummingbirdv1.p.mashape.com/users/${userName}/library?status=currently-watching`,
			method: 'GET',
			...
			let watching = json.map(entry => {
				...
				return {
					...
					url: anime.url,
					id: arn.getAnimeIdBySimilarTitle(anime.title),
			...
			watching.forEach(anime => {
				tasks.push(arn.getAnimeIdBySimilarTitle(anime).then(match => {
